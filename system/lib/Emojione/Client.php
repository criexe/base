...
class Client implements ClientInterface
{
    ...
    public $imagePathPNG        = '/assets/emoji/png/';
    public $imagePathSVG        = '/assets/emoji/svg/';
    ...
    public function shortnameToImageCallback($m)
    {
        if ((!is_array($m)) || (!isset($m[1])) || (empty($m[1]))) {
            ...
            if ($this->imageType == 'png')
            {
                ...
                {
                    return '<img class="emoji emojione" alt="'.$alt.'" src="'.$this->imagePathPNG.$filename.'.png'.$this->cacheBustParam.'"/>';
                }
            }
